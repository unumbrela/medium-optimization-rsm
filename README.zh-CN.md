# Medium-Optim — 基于混合代理建模与贝叶斯优化的培养基设计
...
本项目是响应面方法学的一套可端到端运行的最小化实现 (≈ 300 行)，不依赖 sklearn / GPy /
GPyTorch 等任何高斯过程库，所有数学公式都直接以 NumPy 写出，便于阅读和二次开发。
...
得到趋势项 $f_{\mathrm{trend}}(\mathbf{x}) = \boldsymbol{\Phi}(\mathbf{x})\boldsymbol{\beta}$。

### 2.2 局部偏差：高斯过程残差

残差 $r_i = y_i - f_{\mathrm{trend}}(\mathbf{x}_i)$ 用各向异性 RBF 核建模：
...
- $\sigma_f = \max(10^{-6},\ \mathrm{std}(r))$ — 信号幅度
- $\sigma_n = \max(10^{-6},\ 0.02\cdot\mathrm{range}(y))$ — 噪声幅度
...
| 策略 | 公式 | 取向 |
|---|---|---|
| **Mean** (开发) | $\alpha_{\mathrm{Mean}}(\mathbf{x}) = \mu(\mathbf{x})$ | 在已确定区域精挑 |
| **UCB** (探索) | $\alpha_{\mathrm{UCB}}(\mathbf{x}) = \mu(\mathbf{x}) + \kappa\sigma(\mathbf{x})$，其中 $\kappa=2.58$ (99%) | 给不确定区域留一线生机 |

### 2.4 优化器

**L-BFGS-B + 10 次随机重启**，盒约束 $G\in[30,70]$，$T\in[6,30]$。
...
| **L-BFGS-B + 10 重启** | 二阶曲率信息收敛快；多重启覆盖绝大多数局部最优，且边际收益在 10 次后递减 |
| **输入标准化** | 直接在原始量纲下取 $\ell=1$ 几乎等于无平滑；先 z-score 再用单位长度尺度更合理 |
| **Cholesky + jitter** | 解 $\mathbf{K}+\sigma_n^{2}\mathbf{I}$ 比直接求逆更稳，且数值上自带正定保证 |
